...
class BGNNPredictor:
    ...
    device="cpu"
    bgnn = BGNNPredictor(gnn_model,device)
...
#         predictions = torch.from_numpy(predictions).to(self.device)
    ...
    def init_node_features(self, X):
        node_features = torch.empty(X.shape[0], self.in_dim, requires_grad=True, device="cpu")
        if self.append_gbdt_pred:
    ...
    def train_model(self, input_nodes, seeds, blocks, labels, device, optimizer):
...
#         input_nodes={k : e.to(device) for k, e in input_nodes.items()}
    ...
    def train_and_evaluate(self, loader,labels, device, optimizer, gnn_passes_per_epoch):
        
        loss = None

        for _ in range(gnn_passes_per_epoch):
            for step, (input_nodes, seeds, blocks) in enumerate(loader):
                loss = self.train_model(input_nodes, seeds, blocks, labels, device, optimizer)


        self.model.eval()
        
        train_metrics = self.evaluate_model(loader, labels, device)
    ...
    def evaluate_model(self, loader, labels, device):
        self.model.eval()
        fin_targets=[]
        fin_outputs=[]
        losses=[]
...
#             graph.ndata['node_features']=node_features.to(self.device)
            ...
            train_metric = self.train_and_evaluate(train_loader, y, self.device, optimizer, self.backprop_per_epoch)
            ...
            val_loader=dgl.dataloading.NodeDataLoader(graph.to("cpu"),torch.tensor(val_mask),\
                                                        sampler,batch_size=batch_size, shuffle=True)        
            val_metric = self.train_and_evaluate(train_loader, y, self.device, optimizer, self.backprop_per_epoch)
            
            test_loader=dgl.dataloading.NodeDataLoader(graph.to("cpu"),torch.tensor(test_mask),\
                                                        sampler,batch_size=batch_size, shuffle=True)        
            test_metric = self.train_and_evaluate(test_loader, y, self.device, optimizer, self.backprop_per_epoch)
